<script>
	import Dashboard from '../blocks/Dashboard.svelte';
	import BlockView from './BlockView.svelte';
	import { on_toggle, nav_enter } from '../ui/Place.svelte';

	const snippet = `<!-- Copy src/components/blocks/Dashboard.svelte.
     Mobile overlay and desktop collapse use a few lines of $state. -->
<div class="shell">
	<aside class="sidebar">…team dropdown, search, groups, sub links, badge, user dropdown…</aside>
	<div class="shell-main">
		<header class="topbar">…fold + menu + breadcrumb…</header>
		<div class="shell-body">
			<!-- three .card metrics in .section + col-1-3 -->
			<!-- .table-wrap > table.table -->
		</div>
	</div>
</div>`;

	// Keep in sync with src/components/blocks/Dashboard.svelte. Alumna cannot import the block as text.
...
{#snippet site_nav(prefix)}
	<nav class="nav-menu" aria-label="Site">
		<button
			class="nav-menu-trigger"
			type="button"
			popovertarget={prefix + '-components'}
			aria-expanded="false"
			onmouseenter={nav_enter}
		>Components</button>
		<div
		...
		>
			<div class="nav-menu-grid">
				<a class="nav-menu-item" href="/components/button">
					<span class="nav-menu-item-title">Button</span>
				</a>
				<a class="nav-menu-item" href="/components/card">
					<span class="nav-menu-item-title">Card</span>
				</a>
				<a class="nav-menu-item" href="/components/table">
					<span class="nav-menu-item-title">Table</span>
				</a>
			</div>
		</div>
		<button
		...
		>
			<div class="nav-menu-grid">
				<a class="nav-menu-item" href="/blocks/dashboard">
					<span class="nav-menu-item-title">Dashboard</span>
				</a>
				<a class="nav-menu-item" href="/blocks/login">
					<span class="nav-menu-item-title">Login</span>
				</a>
			</div>
		</div>
		<a class="nav-menu-link" href="/">Docs</a>
	</nav>
{/snippet}

{#snippet scroll_fill()}
	<div class="stack" style="--stack-gap: 1rem; padding: 1rem; min-height: 28rem">
		<p>Scroll this well. The in-flow topbar leaves; the pinned bar stays.</p>
		<div class="card">
			<div class="card-header">
				<div class="card-description">Pipeline</div>
				<div class="card-title" style="font-size: var(--text-2xl); font-weight: 600; font-variant-numeric: tabular-nums">42</div>
			</div>
			<div class="card-footer" style="color: var(--muted-foreground)">Open deals this week</div>
		</div>
		<div class="card">
			<div class="card-header">
				<div class="card-description">Churn</div>
				<div class="card-title" style="font-size: var(--text-2xl); font-weight: 600; font-variant-numeric: tabular-nums">1.8%</div>
			</div>
			<div class="card-footer" style="color: var(--muted-foreground)">Last thirty days</div>
		</div>
		<p>More copy so the column is taller than the well.</p>
		<p>Keep going until the topbar has to move or stick.</p>
		<p>End of the demo column.</p>
	</div>
{/snippet}
...
<div class="box">
	<div class="stack" style="--stack-gap: 1.5rem">
		<div class="stack" style="--stack-gap: 0.5rem">
			<h1>Dashboard</h1>
			<p>
				App shell after shadcn-svelte <code>sidebar-07</code>, metric cards after
				<code>dashboard-01</code> (no charts). Desktop: the fold button sets
				<code>data-collapsed</code> (icons stay, labels hide; hover an icon for a
				tooltip). Below 40rem the menu button opens an overlay; icon-collapse
				does not apply. Header is a team
				<a href="/components/dropdown">dropdown</a> (inline SVG marks). Footer
				is a user dropdown (Account, Billing, Log out). Composes
				<a href="/components/sidebar">Sidebar</a>,
				<a href="/components/card">Card</a>,
				<a href="/components/button">Button</a>,
				<a href="/components/breadcrumb">Breadcrumb</a>,
				<a href="/components/table">Table</a>,
				<a href="/components/dropdown">Dropdown</a>, and
				<a href="/components/tooltip">Tooltip</a>.
				Outline is the map; Full is <code>src/components/blocks/Dashboard.svelte</code>.
				Two extra examples below the block put a
				<a href="/components/navigation-menu">navigation menu</a>
				in the topbar: in the scrolling column it leaves; with
				<code>data-pin</code> it stays.
			</p>
			<nav class="toc" aria-label="On this page">
				<ol>
					<li><a href="#preview">Preview</a></li>
					<li><a href="#in-flow">In flow</a></li>
		...
		<div class="stack" style="--stack-gap: 0.75rem">
			...
			<p>
				...
				scroll. Same <code>.nav-menu</code> pattern as the docs chrome
				(a short stand-in, not the full grid).
			</p>
			<div class="docs-preview docs-scroll-well" id="dash-topbar-flow">
				<header class="topbar">
					{@render site_nav('dash-flow')}
				</header>
				{@render scroll_fill()}
			</div>
		</div>
		...
		<div class="stack" style="--stack-gap: 0.75rem">
			...
			<p>
				<code>data-pin</code> on <code>.topbar</code> sticks it to the top
				of the well. Not a class named <code>sticky</code>. Pin structure
				is in alumna-css; background is Vega skin here.
			</p>
			<div class="docs-preview docs-scroll-well" id="dash-topbar-pin">
				<header class="topbar" data-pin>
					{@render site_nav('dash-pin')}
				</header>
				{@render scroll_fill()}
			</div>
		</div>
